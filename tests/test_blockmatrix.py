...
class TestGetBlockFromMatrix(unittest.TestCase):
    def setUp(self):
        ...
        self.a,self.b,self.c,self.d,self.e,self.f,self.g,self.h,self.i = [j*np.ones([self.dim_block,self.dim_block]) for j in range(9)]

        matrix = np.bmat([[self.a,self.b,self.c],[self.d,self.e,self.f],[self.g,self.h,self.i]])
